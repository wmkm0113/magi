...
public final class CassandraDialectImpl extends DistributeDialect<BatchStatementBuilder> {
    ...
    GeneratedCommand createKeyspace(final String keyspaceName, final List<ServerInfo> serverList) {
        ...
        commandBuilder.append("}");
        return new GeneratedCommand(commandBuilder.toString(), Collections.emptyList(), Map.of(), Map.of());
    }
     ...
     * <span class="zh-CN">生成的CQL命令</span>
     */
    GeneratedCommand insertCommand(@Nonnull final String keyspaceName, @Nonnull final TableDefine tableDefine,
                                   @Nonnull final Map<String, Object> dataMap) {
        String keyspace = StringUtils.isEmpty(tableDefine.getCatalog()) ? keyspaceName : tableDefine.getCatalog();
        StringBuilder commandBuilder = new StringBuilder("INSERT INTO ")
                .append(this.nameCase(keyspace))
                .append(BrainCommons.DEFAULT_NAME_SPLIT)
                .append(this.nameCase(tableDefine.getTableName()));
        ...
        commandBuilder.append(BrainCommons.BRACKETS_BEGIN)
                ...
                .append(BrainCommons.BRACKETS_END);
        return new GeneratedCommand(commandBuilder.toString(), parameters, jdbcTypeMap, keyMap);
    }
     ...
     * <span class="zh-CN">生成的CQL命令</span>
     */
    GeneratedCommand updateCommand(@Nonnull final String keyspaceName, @Nonnull final TableDefine tableDefine,
     ...
     * <span class="zh-CN">生成的CQL命令</span>
     */
    GeneratedCommand deleteCommand(@Nonnull final String keyspaceName, @Nonnull final TableDefine tableDefine,
     ...
     * <span class="zh-CN">生成的CQL命令</span>
     */
    GeneratedCommand queryCommand(@Nonnull final String keyspaceName, @Nonnull final TableDefine tableDefine,
        ...
        if (StringUtils.isEmpty(columns)) {
			...
			for (String columnName : StringUtils.tokenizeToStringArray(columns, BrainCommons.DEFAULT_SPLIT_CHARACTER)) {
    ...
    GeneratedCommand queryCommand(@Nonnull final String keyspaceName, @Nonnull final QueryInfo queryInfo)
            throws SQLException {
	    String tableName = this.tableName(queryInfo.getQueryFrom());
	    ...
	    for (QueryItem abstractItem : queryInfo.getItemList()) {
			...
			if (StringUtils.isEmpty(abstractItem.getAliasName())) {
				if (abstractItem instanceof ColumnItem) {
					aliasName = ((ColumnItem) abstractItem).getColumnName();
					jdbcTypeMap.put(((ColumnItem) abstractItem).getColumnName(), abstractItem.getJdbcType());
				}
			} else {
				aliasName = abstractItem.getAliasName();
				jdbcTypeMap.put(abstractItem.getAliasName(), abstractItem.getJdbcType());
			}
			if (StringUtils.notBlank(aliasName)) {
				String columnLabel = this.nameCase(aliasName);
				jdbcTypeMap.put(columnLabel, abstractItem.getJdbcType());
				keyMap.put(columnLabel, aliasName);
			}
	    }
	    ...
	    String whereClause = this.whereClause(tableName, queryInfo.getConditionList(), values);
	    ...
	    String orderBy = this.orderBy(queryInfo.getOrderByList());
	    if (StringUtils.notBlank(orderBy)) {
		    commandBuilder.append(" ORDER BY ").append(orderBy);
	    }

	    int limitValue = this.limitValue(queryInfo.getPageNo(), queryInfo.getPageLimit());
	    ...
	    return new GeneratedCommand(commandBuilder.toString(), values, jdbcTypeMap, keyMap);
    }
	...
	/**
	 * <h3 class="en-US">Generate data query total commands</h3>
	 * <h3 class="zh-CN">生成数据查询命令</h3>
	 *
	 * @param keyspaceName <span class="en-US">Keyspace name</span>
	 *                     <span class="zh-CN">键空间名称</span>
	 ...
	 *                      <span class="zh-CN">生成CQL命令时出现错误</span>
	 */
	GeneratedCommand queryTotalCommand(@Nonnull final String keyspaceName, @Nonnull final QueryInfo queryInfo)
			throws SQLException {
		String tableName = this.tableName(queryInfo.getQueryFrom());
		if (StringUtils.isEmpty(tableName)) {
			throw new MultilingualSQLException(0x00DB00010020L);
		}
		StringBuilder commandBuilder = new StringBuilder("SELECT COUNT(1) AS COUNT FROM ")
				.append(this.nameCase(keyspaceName))
				.append(BrainCommons.DEFAULT_NAME_SPLIT)
				.append(this.nameCase(tableName));
		...
		return new GeneratedCommand(commandBuilder.toString(), values, Map.of(), Map.of());
	}
     ...
     *                      <span class="zh-CN">生成CQL命令时出现错误</span>
     */
    String queryItem(final QueryItem abstractItem, final List<Object> values) throws SQLException {
    ...
    GeneratedCommand truncateTable(final String tableName) {
        return new GeneratedCommand("TRUNCATE TABLE " + tableName, Collections.emptyList(), Map.of(), Map.of());
    }
    ...
    GeneratedCommand dropTable(final String tableName) {
        return new GeneratedCommand("DROP TABLE IF EXISTS " + tableName, Collections.emptyList(), Map.of(), Map.of());
    }
        ...
        switch (jdbcType) {
            ...
            case Types.TIMESTAMP:
				if (object instanceof java.sql.Date) {
					return new Date(((java.sql.Date) object).getTime()).toInstant();
				}
	            if (object instanceof java.sql.Time) {
		            return new Date(((java.sql.Time) object).getTime()).toInstant();
	            }
