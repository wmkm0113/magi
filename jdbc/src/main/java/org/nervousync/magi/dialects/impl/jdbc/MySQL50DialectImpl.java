...
/**
 * <h2 class="en-US">MySQL 5.0 database dialect implementation class</h2>
 * <h2 class="zh-CN">MySQL 5.0数据库方言实现类</h2>
 *
 * @author Steven Wee	<a href="mailto:wmkm0113@gmail.com">wmkm0113@gmail.com</a>
 * @version $Revision: 1.0.0 $ $Date: Nov 15, 2022 09:41:52 $
 */
@Provider(name = "MySQL5", titleKey = "MySQL5_Dialect_Title", descriptionKey = "MySQL_Dialect_Description")
@SchemaDialect(supportJoin = true,
		types = {
				...
				@DataType(code = Types.DOUBLE, type = "REAL({precision})"),
...
public class MySQL50DialectImpl extends JdbcDialect {

	/**
	 * <h3 class="en-US">Constructor method for MySQL 5.0 database dialect implementation class</h3>
	 * <h3 class="zh-CN">MySQL 5.0数据库方言实现类的构造方法</h3>
	 *
	 ...
	 */
	public MySQL50DialectImpl() throws DialectException {
	...
	public Properties properties(final TrustStore trustStore, final Authentication authentication) {
		...
		Optional.ofNullable(trustStore)
				.ifPresent(store -> {
					properties.put("trustCertificateKeyStoreUrl", store.getTrustStorePath());
					properties.put("trustCertificateKeyStorePassword", store.getTrustStorePassword());
				});
		Optional.ofNullable(authentication)
				...
				.ifPresent(tsAuthentication -> {
					properties.put("clientCertificateKeyStoreUrl", tsAuthentication.getTrustStorePath());
					properties.put("clientCertificateKeyStorePassword", tsAuthentication.getTrustStorePassword());
				});
		return properties;
	}
