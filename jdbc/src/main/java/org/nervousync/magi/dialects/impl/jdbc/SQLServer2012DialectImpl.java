...
public class SQLServer2012DialectImpl extends SQLServer2000DialectImpl {
	...
	public Properties properties(final TrustStore trustStore, final Authentication authentication) {
		...
		Optional.ofNullable(trustStore)
				.ifPresent(store -> {
					properties.put("trustStore", store.getTrustStorePath());
					properties.put("trustStorePassword", store.getTrustStorePassword());
				});
		Optional.ofNullable(authentication)
				...
				.ifPresent(tsAuthentication -> {
					properties.put("clientKey", tsAuthentication.getTrustStorePath());
					properties.put("clientKeyPassword", tsAuthentication.getTrustStorePassword());
				});
		return properties;
	}
}
